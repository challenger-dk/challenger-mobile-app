...
export default function ChallengeDetailScreen() {
  ...
  const isUserParticipating =
    currentUser && challenge.users.some((user) => user.id === currentUser.id);
        ...
        <View className="px-6 pb-8">
          ...
          <View className="bg-[#A0522D] rounded-lg flex-row items-center justify-center py-5 mb-6 relative">
            <View className="flex-1 items-center justify-center">
              <Text className="text-text font-bold text-6xl">
                {team1Count || teamSize || '?'}
              </Text>
            </View>
            <View>
              <Text className="text-text text-4xl font-medium italic z-10">
                vs
              </Text>
            </View>
                ...
                <View className="flex-1">
                  <View className="bg-surface rounded-xl p-4 relative overflow-hidden">
                    <View className="absolute top-0 right-0 opacity-5">
                      <Ionicons
                        name={getSportIcon(challenge.sport) as any}
                        size={120}
                        color="#ffffff"
                      />
                    </View>
                    <Text className="text-text text-lg font-semibold mb-3 relative z-10">
                      {team1?.name || 'Team A'}
                    </Text>
                    <View className="relative z-10">
                      ...
                      {team1 &&
                        ...
                        Array.from({
                          length: teamSize - team1.users.length,
                        }).map((_, idx) => (
                  ...
                  <View className="bg-surface rounded-xl p-4 relative overflow-hidden">
                    ...
                    <View className="relative z-10">
                      ...
                      {team2 &&
                        ...
                        Array.from({
                          length: teamSize - team2.users.length,
                        }).map((_, idx) => (
                          <View key={`empty-${idx}`} className="mb-2">
                            <Text className="text-text-muted text-sm italic">
                              Ledig plads
              ...
              <View className="bg-surface rounded-xl p-4 relative overflow-hidden">
                ...
                <View className="relative z-10">
                  ...
                  {allUsers.length < totalSlots &&
                    Array.from({ length: totalSlots - allUsers.length }).map(
                      (_, idx) => (
                        <View key={`empty-${idx}`} className="mb-2">
                          <Text className="text-text-muted text-sm italic">
                            Ledig plads
          ...
          <View className="bg-surface rounded-xl p-5 mb-6">
            ...
            <View className="flex-row items-center justify-between mb-5 pb-5 border-b border-black/20">
              ...
              <View className="flex-row gap-2">
                <View className="bg-background rounded-full px-4 py-2">
            ...
            <View className="flex-row items-center justify-between mb-5 pb-5 border-b border-black/20">
              ...
              <View
                className={`rounded-full px-4 py-2 ${challenge.is_indoor ? 'bg-blue-600' : 'bg-green-600'}`}
              >
            ...
            {challenge.comment && (
              <View className="flex-row items-start justify-between">
                ...
                <View className="bg-background rounded-xl px-4 py-3 flex-1 ml-4 max-w-[70%]">
                  <Text className="text-text text-sm">{challenge.comment}</Text>
                </View>
              </View>
            )}
          </View>
          ...
          <View className="gap-3">
            {!challenge.is_completed && (
              ...
              >
                <Text
                  className={`text-lg font-bold ${isUserParticipating ? 'text-red-400' : 'text-background'}`}
                >
                  {isJoining
                    ? 'Opdaterer...'
                    : !currentUser
            ...
            {hasTwoTeams && (
              <Pressable
                className="bg-surface rounded-xl p-4 items-center border border-[#3A3A3A]"
                onPress={() => router.back()}
              >
                <Text className="text-text text-base font-medium">
                  Se hold detaljer
