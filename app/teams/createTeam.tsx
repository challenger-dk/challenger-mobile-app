...
export default function CreateTeamScreen() {
      ...
      const loadUsers = async () => {
        ...
        try {
          ...
          setAllUsers(
            users.filter(
              (u: { id: string | number }) =>
                !invitedIds.has(u.id) && u.id !== user?.id
            )
          );
        } catch (err) {
  ...
  const handleInvite = async (invitee: User) => {
    ...
    const inviteeId =
      typeof invitee.id === 'string' ? parseInt(invitee.id, 10) : invitee.id;
            ...
            <ScrollView className="w-full max-w-sm" style={{ maxHeight: 300 }}>
              ...
              {isLoadingUsers ? (
                ...
                allUsers.map((user) => (
                  <View
                    key={user.id}
                    className="flex-row items-center justify-between bg-surface p-3 rounded-lg mb-2"
                  >
