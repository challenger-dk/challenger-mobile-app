...
export const ParticipantCountDisplay = ({
  joinedParticipants,
  totalParticipants,
  ...
  const participationPercentage =
    totalParticipants > 0
      ? Math.min(joinedParticipants / totalParticipants, 1)
      : 0;
  const topPercentage = isOverCapacity ? 100 : participationPercentage * 100;
  const bottomPercentage = isOverCapacity
    ? 0
    : (1 - participationPercentage) * 100;
    ...
    <View className="w-[17.5%] relative self-stretch">
      <View className="absolute -top-5 -bottom-3 left-0 right-0 flex-col">
        ...
        <View className="w-[100%] items-center justify-center absolute top-1/2 left-0 -translate-y-1/2 z-10">
          ...
          <Text className="text-text font-black text-sm tracking-wider z-10 px-2 left-1">
            /
          </Text>
        </View>
        <View
      ...
      <View className="absolute right-[-25px] top-[48%] -translate-y-1/2 z-20">
        <View className="relative">
          <View
            className="absolute inset-0 bg-warning rounded-full border-2 border-background"
            style={{ width: 32, height: 32, left: 2, top: 2 }}
          />
          <Ionicons
